...
impl TryFrom<&Value> for OrtexTensor {
    ...
    fn try_from(e: &Value) -> Result<Self, Self::Error> {
        let dtype: ValueType = e.dtype().clone();
        let ty = match dtype {
            ValueType::Tensor { ty: t, .. } => t,
        ...
        let tensor = match ty {
            ...
            TensorElementType::Uint8 => OrtexTensor::u8(e.try_extract_tensor::<u8>()?.into_owned()),
            ...
            TensorElementType::Int8 => OrtexTensor::s8(e.try_extract_tensor::<i8>()?.into_owned()),
