defmodule Ortex do
  ...
  @doc """
  Loads a model with structured execution-provider options.

  CoreML providers accept the ONNX Runtime option names normalized by
  `Ortex.Model`. Set `:profile_prefix` to enable ONNX Runtime profiling; call
  `end_profiling/1` after inference to flush and return the generated trace.

  ## Example

      Ortex.load_with_options(
        "model.onnx",
        coreml: [
        ...
        profile_prefix: "/tmp/ortex-coreml"
      )

  """
  defdelegate load_with_options(path, providers, opts \\ []), to: Ortex.Model

  @doc """
  Ends profiling and returns the ONNX Runtime trace path.

  A model can only end profiling once.
  """
  defdelegate end_profiling(model), to: Ortex.Model
